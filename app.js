...
const run = async () => {
    ...
    const memberStat = await proClubsApi.member.getMembersStats('_SZLEQTI3zZwcZkwQxFg1Q!!:NTumR8n2HNqVIPsyuocEhQ!!')
    console.log({ clubId, resp, membersStats, matches, seasonalStats })
    const data = aggregate({ clubId, resp, membersStats, matches, seasonalStats })
...
const setMatch = (match, clubId) => {
    ...
    const { assists, goals, passattempts, passesmade, shots, redcards } = aggregate[clubId];

    return {
        timeAgo,
        team: _matchDataForTeam({ players: players[clubId], clubs: clubs[clubId], aggregate: aggregate[clubId], id: clubId }),
        opponent: _matchDataForTeam({ players: players[opponentId], clubs: clubs[opponentId], aggregate: aggregate[opponentId], id: opponentId }),
        result: {
...
const _matchDataForTeam = ({ players, clubs, aggregate, id }) => ({
    ...
    playersDetails: Object.entries(players).map(player => _playerDetailsInMatch(player[0], player[1]))
...
const _teamData = (data) => {
    ...
    return { gameNumber, goals, goalsAgainst, losses, ties, details, name, teamId }

}
const _playerDetailsInMatch = (blazeId, data) => {
    const { assists, goals, passattempts, passesmade, playername, pos, rating, shots, tackleattempts, tacklesmade } = data
    return {
        blazeId,
        assists,
        goals,
        passattempts,
        passesmade, playername, pos, rating, shots, tackleattempts, tacklesmade,
        passAccuracy: (passesmade / passattempts),
        tackleAccuracy: (tacklesmade / tackleattempts)

    }
}
const _getResult = ({ goals, goalsAgainst }) => (goals > goalsAgainst) ? 'win' : (goalsAgainst < goals) ? 'lose' : 'tie';
//run();
